...
class FeedBackController extends CommonController {
	...
	public function index(){
		...
		$feedback_list=M('message')->field('s.id,s.message_info,s.state,s.add_time,m.member_name,s.admire') ->table(array('sto_member'=>'m','sto_message'=>'s')) ->where('s.user_id=m.id')->limit($Page->firstRow.','.$Page->listRows)->order('add_time desc')->select();
